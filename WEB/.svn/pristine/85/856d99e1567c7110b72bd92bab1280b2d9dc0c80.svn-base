            ...
            return this.each(function(){
                ...
                select_ul.on("click", "li", function () {
                    ...
                    if (val !== $this.val()) {
                        ...
                        if($(this).parents('.select-div').hasClass('style7-select')&&$(".select-div .select_ul li").eq(0).html()&&(li.text()!='立项类型')&&(li.text()!='投资类型')&&(li.text()!='项目类型')){
                            select_text.text(li.parents('.select-div').find(".select_ul li").eq(0).html()+':'+li.text());
                            li.parents('.select').addClass('xz');
                        }else{
                            select_text.text(li.text());
                            li.parents('.select').removeClass('xz');
                        }
