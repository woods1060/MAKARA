from pwn import *
from ScanBUG import ArbRW, Stack, Resgs, Heap, Format
...
# 寻找寄存器错误
def find_error_regs_repeat(filename,argv,entry_addr,limit=None):
...
# 寻找格式化字符串漏洞
def find_format_repeat(filename,argv,entry_addr,limit=None):
...
# 寻找堆漏洞
def find_heap_vul_repeat(filename,argv,entry_addr,limit=None):
...
def main(args):
    ...
    find_stack_overflow(filename,argv,entry_addr,limit_time)
    find_arbitrary(filename,argv,entry_addr,limit_time)
    find_error_regs(filename,argv,entry_addr,limit_time)
    find_format(filename,argv,entry_addr,limit_time)
    find_heap_vul(filename,argv,entry_addr,limit_time)
